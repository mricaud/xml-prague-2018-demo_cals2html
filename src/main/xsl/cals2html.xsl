...
<xsl:stylesheet version="2.0"
  ...
  <!--PARAMETERS-->
  <!--common-->
  <xsl:param name="elsLib:cals2html.log.uri" select="resolve-uri('log/', base-uri())" as="xs:string"/>
  <xsl:param name="elsLib:cals2html.debug" select="false()" as="xs:boolean"/>
  <!--structure-->
  <xsl:param name="elsLib:cals2html.html-version" select="5" as="xs:double"/> <!--4 or 5 for example-->
  <xsl:param name="elsLib:cals2html.use-style-insteadOf-class" select="true()" as="xs:boolean"/>
  <xsl:param name="elsLib:cals2html-keep-unmatched-attributes" select="false()" as="xs:boolean"/>
  <xsl:param name="elsLib:cals2html-unmatched-attributes-prefix" select="'data-cals2html-'" as="xs:string"/>
  <xsl:param name="elsLib:cals2html.compute-column-width-as-width-attribute" select="true()" as="xs:boolean"/> <!--@width is used for html4 output-->
  <xsl:param name="elsLib:cals2html.compute-column-width-within-colgroup" select="true()" as="xs:boolean"/>
  <!--If the number of columns is greater than $nb-cols-max-before-font-reduction then the font needs to be reduced-->
  <xsl:param name="elsLib:cals2html.nb-cols-max-before-font-reduction" select="8" as="xs:integer"/>
  <xsl:param name="elsLib:cals2html.nb-cols-max-before-large-font-reduction" select="14" as="xs:integer"/>
  <!--default colsep/rowsep-->
  <xsl:param name="elsLib:cals2html.default-colsep" select="'yes'" as="xs:string"/>
  <xsl:param name="elsLib:cals2html.default-rowsep" select="'yes'" as="xs:string"/>
  <!--default align/valign-->
  <xsl:param name="elsLib:cals2html.default-tgroup-align" select="'left'" as="xs:string"/>
  <xsl:param name="elsLib:cals2html.default-td-align" select="'left'" as="xs:string"/><!--default browser value-->
  <xsl:param name="elsLib:cals2html.default-th-align" select="'center'" as="xs:string"/><!--default browser value-->
  <xsl:param name="elsLib:cals2html.default-tgroup-valign" select="'middle'" as="xs:string"/>
  <xsl:param name="elsLib:cals2html.default-td-valign" select="'middle'" as="xs:string"/><!--default browser value-->
  <xsl:param name="elsLib:cals2html.default-th-valign" select="'middle'" as="xs:string"/><!--default browser value-->
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="/" mode="elsLib:cals2html"/>
  </xsl:template>
  ...
  <xsl:template match="/" mode="elsLib:cals2html">
    <xsl:variable name="step" select="." as="document-node()"/>
    <!--STEP1 : normalize cals table-->
    <xsl:variable name="step" as="document-node()">
      <xsl:document>
        <xsl:apply-templates select="$step" mode="elsLib:normalizeCalsTable"/>
      </xsl:document>
    </xsl:variable>
    <xsl:if test="$elsLib:cals2html.debug">
      <xsl:variable name="step.log.uri" select="resolve-uri('cals2html.step1.xml', $elsLib:cals2html.log.uri)" as="xs:anyURI"/>
    ...
    <xsl:variable name="step" as="document-node()">
      <xsl:document>
        <xsl:apply-templates select="$step" mode="elsLib:cals2html.main"/>
      </xsl:document>
    </xsl:variable>
    <xsl:if test="$elsLib:cals2html.debug">
      <xsl:variable name="step.log.uri" select="resolve-uri('cals2html.step2.xml', $elsLib:cals2html.log.uri)" as="xs:anyURI"/>
      <xsl:message>[INFO] writing <xsl:value-of select="$step.log.uri"/></xsl:message>
      <xsl:result-document href="{$step.log.uri}">
        <xsl:sequence select="$step"/>
      </xsl:result-document>
    ...
    <xsl:variable name="step" as="document-node()">
      <xsl:choose>
        <xsl:when test="$elsLib:cals2html.use-style-insteadOf-class">
          <xsl:document>
            <xsl:apply-templates select="$step" mode="elsLib:cals2html.class2style"/>
          </xsl:document>
        </xsl:when>
        <xsl:otherwise>
          <xsl:sequence select="$step"/>
    ...
    <xsl:if test="$elsLib:cals2html.debug">
      <xsl:variable name="step.log.uri" select="resolve-uri('cals2html.step3.xml', $elsLib:cals2html.log.uri)" as="xs:anyURI"/>
      <xsl:message>[INFO] writing <xsl:value-of select="$step.log.uri"/></xsl:message>
      <xsl:result-document href="{$step.log.uri}">
        <xsl:sequence select="$step"/>
      </xsl:result-document>
  ...
  <xsl:template match="table" mode="elsLib:cals2html.main">
    ...
    <div class="cals_table">
      ...
      <xsl:apply-templates select="@*" mode="elsLib:cals2html.attributes"/>
      <xsl:apply-templates mode="#current"/>
    </div>
  </xsl:template>
  
  <xsl:template match="caption" mode="elsLib:cals2html.main">
  ...
  <xsl:template match="tgroup" mode="elsLib:cals2html.main">
    <table>
      ...
      <xsl:if test="$elsLib:cals2html.compute-column-width-within-colgroup">
        ...
        <colgroup>
          <xsl:variable name="total-colwidth-sum" select="elsLib:cals2html.cals_sum-colwidths(colspec)" as="xs:double"/>
          <xsl:for-each select="colspec">
            <xsl:variable name="current-colwidth" select="elsLib:cals2html.cals_sum-colwidths(.)" as="xs:double"/>
            <col>
              <xsl:if test="$total-colwidth-sum != 0">
                <xsl:call-template name="elsLib:cals2html.add-column-width">
      ...
      <xsl:if test="normalize-space(@cols) != '' and not(normalize-space(@cols) castable as xs:integer)">
        <xsl:message terminate="no">[ERROR][elsLib:cals2html] @cols="<xsl:value-of select="@cols"/>" is not an integer</xsl:message>
      </xsl:if>
      <xsl:apply-templates mode="elsLib:cals2html.main" select="thead"/>
      <xsl:if test="tfoot">
        <tfoot>
  ...
  <xsl:template match="thead" mode="elsLib:cals2html.main">
    <thead>
      <xsl:apply-templates mode="#current"/>
    </thead>
  </xsl:template>
  
  <xsl:template match="colspec" mode="elsLib:cals2html.main">
    <!-- no operation -->
  </xsl:template>
  
  <xsl:template match="spanspec" mode="elsLib:cals2html.main">
    <!-- no operation -->
  </xsl:template>
  
  <!-- Table Foot -->
  <!-- CALS MODEL : tfoot ::= colspec*,row+ -->
  <xsl:template match="tfoot" mode="elsLib:cals2html.main">
    <xsl:apply-templates select="*" mode="#current"/>
  </xsl:template>
  
  <!-- Table bocy -->
  <!-- CALS MODEL : tbody ::= row+-->
  <xsl:template match="tbody" mode="elsLib:cals2html.main">
    <tbody>
      <xsl:apply-templates mode="#current"/>
    </tbody>
  </xsl:template>
  ...
  <xsl:template match="row" mode="elsLib:cals2html.main">
    <tr>
      <xsl:apply-templates select="@*" mode="elsLib:cals2html.attributes"/> 
      <xsl:variable name="class.tmp" as="xs:string*">
        <!--<xsl:if test="$elsLib:cals2html.add-odd-even-class">
  ...
  <xsl:template match="entry" mode="elsLib:cals2html.main">
    ...
    <xsl:variable name="current-colspec-list" as="element(colspec)*">
      <xsl:choose>
        ...
        <xsl:when test="@namest and @nameend">
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              ...
              <xsl:variable name="colnumst" select="elsLib:cals2html.get-colnum($entry, $namest.colspec)" as="xs:integer"/>
              <xsl:variable name="colnumend" select="elsLib:cals2html.get-colnum($entry, $nameend.colspec)" as="xs:integer"/>
              <xsl:sequence 
                select="$current-tgroup/colspec
                [elsLib:cals2html.get-colnum($entry, .) ge $colnumst]
                [elsLib:cals2html.get-colnum($entry, .) le $colnumend]"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
        ...
        <xsl:when test="position() > 1 and ../entry[@colname]">
          <xsl:message>[ERROR][cals2html.xsl] Unable to get colspec for this entry. @colname might be missing ?</xsl:message>
        </xsl:when>
        <xsl:when test="position() > 1 and ../entry[(position() lt last() and (@namest and @nameend))]">
          <xsl:message>[ERROR][cals2html.xsl] Unable to get colspec for this entry. Too much columns</xsl:message>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:variable name="colsep-current" as="xs:string">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$elsLib:cals2html.default-colsep"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="rowsep-current" as="xs:string">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$elsLib:cals2html.default-rowsep"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="align-current" as="xs:string">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$elsLib:cals2html.default-tgroup-align"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="valign-current" as="xs:string">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$elsLib:cals2html.default-tgroup-valign"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <xsl:element name="{$name}">
      ...
      <xsl:variable name="class.tmp" as="xs:string*">
        ...
        <xsl:if test="$align-current != (if($name = 'td') then($elsLib:cals2html.default-td-align) else($elsLib:cals2html.default-th-align))">
          <xsl:value-of select="concat('cals_align-', lower-case($align-current))" />
        </xsl:if>
        <xsl:if test="$valign-current != (if($name = 'td') then($elsLib:cals2html.default-td-valign) else($elsLib:cals2html.default-th-valign))">
          <xsl:value-of select="concat('cals_valign-', lower-case($valign-current))" />
        </xsl:if>
        <xsl:if test="not(empty($nb-cols))">
          <xsl:if test="$nb-cols > $elsLib:cals2html.nb-cols-max-before-font-reduction
            and $nb-cols lt $elsLib:cals2html.nb-cols-max-before-large-font-reduction">
            <xsl:text>cals_table-font-reduction</xsl:text>
          </xsl:if>
          <xsl:if test="$nb-cols > $elsLib:cals2html.nb-cols-max-before-large-font-reduction">
      ...
      <xsl:variable name="total-colwidth-sum" select="elsLib:cals2html.cals_sum-colwidths($current-tgroup/colspec)" as="xs:double"/>
      <xsl:variable name="current-colwidth" select="elsLib:cals2html.cals_sum-colwidths($current-colspec-list)" as="xs:double"/>
      <xsl:if test="not($elsLib:cals2html.compute-column-width-within-colgroup)">
        <xsl:call-template name="elsLib:cals2html.add-column-width">
          <xsl:with-param name="width" select="concat(round(($current-colwidth div  $total-colwidth-sum) * 100), '%')" as="xs:string"/>
        </xsl:call-template>
      </xsl:if>
  ...
  <xsl:template match="table/@frame | tgroup/@cols | entry/@namest | entry/@nameend | entry/@colname | entry/@morerows |
    cals:*/@rowsep | cals:*/@colsep | cals:*/@valign | cals:*/@align | table/@pgwide" mode="elsLib:cals2html.attributes"/>
  
  <xsl:template match="table/@orient | table/@tabstyle | table/@shortentry | table/@tocentry | 
    tgroup/@tgroupstyle | entry/@rotate" mode="elsLib:cals2html.attributes" priority="1">
    <!--FIXME : rotate could be done with css3-->
    <xsl:choose>
      <xsl:when test="$elsLib:cals2html.html-version = 5">
  ...
  <xsl:template match="tgroup/@char | tgroup/@charoff | entry/@char | entry/@charoff" 
    mode="elsLib:cals2html.attributes">
    <xsl:if test="$elsLib:cals2html.html-version le 4">
      <xsl:copy-of select="."/>
    </xsl:if>
  </xsl:template>
  
  <!--Attributes to keep as is-->
  <xsl:template match="@xml:* | @id" mode="elsLib:cals2html.attributes">
    <xsl:copy copy-namespaces="no"/>
  </xsl:template>
  
  <xsl:template match="@*" mode="elsLib:cals2html.attributes">
    <xsl:choose>
      <xsl:when test="$elsLib:cals2html-keep-unmatched-attributes">
        <xsl:attribute name="{concat($elsLib:cals2html-unmatched-attributes-prefix, local-name(.))}" select="."/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:message>[ERROR] <xsl:value-of select="name(parent::*)"/>/@<xsl:value-of select="name()"/> unmatched in mode "elsLib:cals2html.attributes"</xsl:message>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  
  <!--copy template-->
  <xsl:template match="node() | @*" mode="elsLib:cals2html.main">
    <xsl:copy copy-namespaces="no">
  ...
  <xsl:function name="elsLib:cals2html.cals_sum-colwidths" as="xs:double">
    <xsl:param name="colspec-list" as="element(colspec)*"/>
    <xsl:sequence select="elsLib:cals2html.cals_sum-colwidths($colspec-list, 0)"/>
  </xsl:function>
  ...
  <xsl:function name="elsLib:cals2html.cals_sum-colwidths" as="xs:double">
    ...
    <xsl:choose>
      <xsl:when test="count($colspec-list) != 0">
        ...
        <xsl:sequence select="elsLib:cals2html.cals_sum-colwidths($colspec-list[position() gt 1], $current-sum + $colwidth.as-number)"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="elsLib:cals2html.add-column-width">
    <xsl:param name="width" required="yes" as="xs:string"/>
    <xsl:choose>
      <xsl:when test="$width = 'NaN%'">
        <xsl:message>[ERROR][cals2html.xsl] Unable to compute width (NaN%)</xsl:message>
      </xsl:when>
      <xsl:when test="$width = '0%'">
        <xsl:message>[WARNING][cals2html.xsl] width=0 will not be computed</xsl:message>
      </xsl:when>
      <xsl:when test="$elsLib:cals2html.compute-column-width-as-width-attribute">
  ...
  <xsl:function name="elsLib:cals2html.get-colnum" as="xs:integer">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:message terminate="yes">[ERROR][cals2html.xsl] Calling elsLib:cals2html.get-colnum() with $colspec empty argument</xsl:message>
  ...
  <xsl:variable name="elsLib:cals2html.class2style.mapping" as="element()">
  ...
  <xsl:template match="html:table[els:hasClass(., 'cals_tgroup')] 
    ...
    mode="elsLib:cals2html.class2style">
    <xsl:copy>
      <xsl:copy-of select="@* except (@class | @style)"/>
      <xsl:variable name="class" select="tokenize(@class, '\s+')[not(. = $elsLib:cals2html.class2style.mapping/entry/@key)]" as="xs:string*"/>
      <xsl:if test="not(empty($class))">
        <xsl:attribute name="class" select="string-join($class, ' ')"/>
      </xsl:if>
      <xsl:variable name="style" as="xs:string*">
        <xsl:sequence select="tokenize(@style, ';')"/>
        <xsl:for-each select="tokenize(@class, '\s+')[. = $elsLib:cals2html.class2style.mapping/entry/@key]">
          <xsl:variable name="val" select="." as="xs:string"/>
          <xsl:sequence select="tokenize($elsLib:cals2html.class2style.mapping/entry[@key = $val], ';')"/>
        </xsl:for-each>
      </xsl:variable>
  ...
  <!--Default copy-->
  <xsl:template match="* | @* | node()" mode="elsLib:cals2html.class2style">
    <xsl:copy>
      <xsl:apply-templates select="@* | node()" mode="#current"/>
    </xsl:copy>
  </xsl:template>
